...
class TodosController(private val todoService: TodoService) {
    ...
    fun getAll(): List<TodoResponse> {
        return todoService.getAll().map {
            TodoResponse(it.id!!, it.description, it.finished, it.label)
        }
    }
    ...
    fun createTodo(@RequestBody newTodoRequest: NewTodoRequest): TodoResponse {
        ...
        return TodoResponse(created.id!!, created.description, created.finished, created.label)
    }

    @PatchMapping("/{id}/label")
    fun assignLabel(@PathVariable id: Int, @RequestBody assignLabelRequest: AssignLabelRequest): TodoResponse {
        val updated = todoService.assignLabel(id, assignLabelRequest)
        return TodoResponse(updated.id!!, updated.description, updated.finished, updated.label)
    }
}