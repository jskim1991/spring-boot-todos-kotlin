...
class DefaultTodoRepository(private val todoJpaRepository: TodoJpaRepository) : TodoRepository {
    ...
    override fun findById(id: Int): Todo? {
        val todoEntity = todoJpaRepository.findById(id)
        return if (todoEntity.isPresent) {
            val entity = todoEntity.get()
    ...
    override fun update(id: Int, description: String): Todo? {
        ...
        return if (todoEntity.isPresent) {
            val entity = todoEntity.get()
            val updated = todoJpaRepository.save(TodoEntity(entity.id, description, entity.finished))
