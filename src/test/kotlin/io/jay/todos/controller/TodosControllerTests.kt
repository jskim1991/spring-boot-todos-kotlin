...
class TodosControllerTests {
    ...
    inner class UpdateTodo {
        val updateTodoRequest = UpdateTodoRequest("Learn Kotlin", true)
        val requestBody = objectMapper.writeValueAsString(updateTodoRequest)

        @Test
        fun `should return 200`() {
            every { mockTodoService.update(any(), any()) } returns Todo(1, "Learn Kotlin", true)
        ...
        @Test
        fun `should call todoService`() {
            every { mockTodoService.update(1, updateTodoRequest) } returns Todo(1, "Learn Kotlin", true)
